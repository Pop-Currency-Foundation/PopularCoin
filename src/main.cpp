...
uint256 hashGenesisBlock("0x00000e1438374d15f55c670fcae8d7cce5456a0b4cfa7429503eaeb46f6d55d4");
...
unsigned int nStakeTargetSpacing = 60;			// 60 sec stake block spacing
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight == 1)
    {
        nSubsidy = 3470000000 * COIN;  // 3.47b coins first block to cover coinswap / redistribution
        return nSubsidy + nFees;
    }
    if (nHeight == 2)
    {
        nSubsidy = 347000000 * COIN;  // 347 Million Coins Second Block 10% for Marketing and Development Bounties
        return nSubsidy + nFees;
    }
    
  if (nHeight >= 3 && nHeight < 400)
    {
		nSubsidy = 0 * COIN;
  ...
  if (nHeight >= 400 && nHeight < 87600) // 6 months
    {
		nSubsidy = 99 * COIN;
		...
		{
			nSubsidy *= 1000;
		} else if (mod_floor(nHeight, 29199) == 0) {
		// Every 2 month or 6 times a year
			nSubsidy *= 10000;
		} else if (mod_floor(nHeight, 14600) == 0) {
		// Every month
			nSubsidy *= 2500;
		} else if (mod_floor(nHeight, 3367) == 0) {
		// Every Week
			nSubsidy *= 500;
		} else if (mod_floor(nHeight, 481) == 0) {
		// Every day
			nSubsidy *= 100;
		} else if (mod_floor(nHeight, 39) == 0) {
		// Every 2 hours or 12 times a day
			nSubsidy *= 25;
		} else if (mod_floor(nHeight, 20) == 0) {
		// 1 time per hour 
			nSubsidy *= 10;
		} else if (mod_floor(nHeight, 11) == 0) {
		// 2 times per hour 
			nSubsidy *= 5;
		} else if (mod_floor(nHeight, 3) == 0) {
		// 7 times per hour equal to 9 blocks
			nSubsidy *= 2;
		}
  ...
  if (nHeight >= 87600 && nHeight < 110000) //6 months
    {
		nSubsidy = 49 * COIN;
  ...
  if (nHeight >= 110000 && nHeight < 256000)
    {
		nSubsidy = 99 * COIN;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "January 28, 2014, V 1.0 PopularCoin Comes To Existence - V 2.0 Gen Block June 14, 2018 1529021473";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1529021473;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3426;

		
        assert(block.hashMerkleRoot == uint256("0x0340c70020fbc67b355e0470b9966f884df60034a577daffbfb464b88d783325"));
...
string GetWarnings(string strFor)
{
    ...
    // ppcoin: should not enter safe mode for longer invalid chain - Uncomment if needed
